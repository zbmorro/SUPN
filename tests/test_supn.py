...
def test_initialization():
    ...
    assert nparams == 50, 'Failure in 1D initialization'

    net = SUPN(max_degree=3, width=10, d=2, space='total_degree')
    nparams = sum([param.numel() for param in net.parameters()])
    assert nparams == 110, 'Failure in 2D initialization'
...
def test_differentiation_2d():
    ...
    u_y_err = ((u_y_approx - derivatives[0, 1, :, 1:-1]).norm() /
               derivatives[0, 0, :, 1:-1].norm()).item()
    assert max(u_x_err, u_y_err) < 5e-5, 'Failure in 2D 1st derivative'
    ...
    assert max(u_xx_err, u_yy_err) < 7e-5, 'Failure in 2D 2nd derivative'
    ...
    assert max(u_xxx_err, u_yyy_err) < 6e-4, 'Failure in 2D 3rd derivative'
    ...
    assert max(u_xxxx_err, u_yyyy_err) < 6e-4, 'Failure in 2D 4rd derivative'
...
def test_precompute_data():
    ...
    assert diff < 1e-13, 'Failure in three-term recurrence'

    net = SUPN(max_degree=5, width=1, d=1, ntrain=x.shape[0])
    net.precompute_data(x)
    assert (net(x) == net(x, recompute=True)).all(), 'Failure precomputing'
...
def test_domain_transform():
    ...
    assert diff < 1e-14, 'Failure in 1D domain transform'
    ...
    assert diff < 1e-13, 'Failure in 2D domain transform'
